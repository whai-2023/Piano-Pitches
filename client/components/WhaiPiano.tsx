...
function WhaiPiano() {
  ...
  const { data: participants, error } = useQuery<ParticipantResponse[]>(
    ['participants'],
    getParticipants
  )
  ...
  function handleKeyClick(key: string) {
    ...
    const participant1 = null
    const participiant2 = null

    if (key.length === 2) {
      participant1 = participants.find(key)
    }
  ...
  if (selectedKey !== null) {
    ...
    useEffect(() => {
      if (participants != undefined) {
        audio.src = participants.participant?.audioURL
        audio.play()
      }
    }, [participants, audio])
